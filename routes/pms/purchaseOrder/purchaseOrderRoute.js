...
const {
  ...
  joiPurchaseLineUpdate
} = require('../../../joiSchemas/pms/purchaseOrder/joiPurchaseLine');
const {
  joiPurchaseHeaderInsert,
  joiPurchaseHeaderUpdate
} = require('../../../joiSchemas/pms/purchaseOrder/joiPurchaseHeader');
...
router.get('/get/header/:Organization_ID', (req, res) => {
  ...
  let searchQuery = `SELECT PO_Header_ID, PO_NO, PO_Date,Payment_Type, Ref_No, po.Remarks, Status,
  ...
  let searchCountQuery = `SELECT count(*) as totalCount from purchase_order_header AS po
  INNER JOIN supplier AS s ON po.Supplier_ID = s.Supplier_ID
	INNER JOIN department AS d ON po.Ship_To_ID = d.Department_ID
  WHERE PO_NO LIKE '%${searchTerm}%' OR PO_Date LIKE '%${searchTerm}%'
  OR s.Supplier_Name LIKE '%${searchTerm}%' OR d.Department_Name LIKE '%${searchTerm}%'
...
router.get('/get/line/:PO_Header_ID', (req, res) => {
  ...
  let sql = `SELECT pl.PO_Line_ID, PO_Header_ID, pl.Item_ID, i.Item_Name, pl.UOM_Name, Item_Qty,
...
router.post('/add/:type', (req, res) => {
  ...
  if (error || error1) {
    ...
    db.query(sql, header, (err, result) => {
      ...
      sql = `INSERT INTO purchase_order_line (Item_ID, UOM_Name, Item_Qty, Price, GST_Per, GST_Amt,
...
router.put('/update/:PO_Header_ID/:type', (req, res) => {
  ...
  if (error) {
    ...
    db.query(sql, [value, PO_Header_ID], (err, result) => {
      ...
      const newEntries = lines.filter(
        line => !line.hasOwnProperty('PO_Line_ID')
      );
      const UpdatedEntries = lines.filter(line =>
        line.hasOwnProperty('PO_Line_ID')
      );
      let newData = [];
      let sql = '';
      let UpdateQuery = '';
      ...
      if (UpdatedEntries.length > 0) {
        ...
        UpdatedEntries.forEach(item => {
          UpdateQuery += db.format(
            'UPDATE purchase_order_line SET ? WHERE PO_Line_ID = ?; ',
            [item, item.PO_Line_ID]
          );
        });
        sql = sql + UpdateQuery;
      }
      ...
      if (newEntries.length > 0) {
        ...
        newEntryQuery = db.format(
          'INSERT INTO purchase_order_line (Item_ID, UOM_Name, Item_Qty, Price, GST_Per, GST_Amt, Total_Amt,Created_By, Creation_Date,Last_Updated_Date, Last_Updated_By, PO_Header_ID) VALUES ? ;',
          [newData]
        );
...
router.delete('/delete/:PO_Header_ID', (req, res) => {
  ...
  let sql = `DELETE from purchase_order_line Where PO_Header_ID = ?;
...
router.delete('/delete/line/:PO_Line_ID', (req, res) => {
  ...
  let sql = `DELETE from purchase_order_line Where PO_Line_ID = ?`;
