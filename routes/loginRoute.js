...
router.post('/login', (req, res) => {
    ...
    if (username && password) {
        db.query('SELECT User_Name ,User_hpassword, User_ID, User_Email, Organization_ID, Branch_ID,Employee_Name,Designation FROM users WHERE User_Name = ?', [username],
            (error, results, fields) => {
                ...
                if (results.length > 0) {
                    const { User_ID, User_hpassword, Organization_ID, Branch_ID, User_Email, User_Name,Employee_Name,Designation } = results[0];
