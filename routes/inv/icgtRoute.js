...
// @access   Private 
    ...
    if(error){
     // console.log(error);
      ...
      pool.query('UPDATE item_category_group_type SET ? Where Item_Cat_Grp_Type_ID = ?',[req.body,req.params.id],(err,result) => {
        if (err) throw err;
...
router.delete('/delete-icgt/:id',(req,res) => {
    pool.query('DELETE from item_category_group_type Where Item_Cat_Grp_Type_ID = ?',[req.params.id],(err,result) => {
      if (err){
       // console.log(err)
