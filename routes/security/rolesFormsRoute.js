...
const {
  joiRolesFormsInsert,
  joiRolesFormsUpdate
} = require('../../joiSchemas/security/joiRoles_forms');
...
router.post('/post', (req, res) => {
  ...
  if (error) {
    ...
    db.query('INSERT INTO roles_forms SET ?', req.body, (err, result) => {
      ...
      return res
        .status(200)
        .send(`Role_form_ID : ${result.insertId} Record Inserted`);
    });
  }
});
...
router.put('/update/:id', (req, res) => {
  ...
  if (error) {
    ...
    db.query(
      'UPDATE roles_forms SET ? Where Role_form_ID = ?',
...
router.delete('/delete/:id', isAuthenticated, (req, res) => {
  db.query(
    'DELETE from roles_forms Where Role_form_ID = ?',
    [req.params.id],
    (err, result) => {
      if (err) {
        console.log(err);
