...
} = require('../../joiSchemas/security/joiRoles_modules');
...
router.post('/post', (req, res) => {
  ...
  if (error) {
    ...
    db.query('INSERT INTO roles_modules SET ?', req.body, (err, result) => {
      ...
      return res
        .status(200)
        .send(`Role_Module_ID : ${result.insertId} Record Inserted`);
    });
  }
});
...
router.put('/update/:id', (req, res) => {
  ...
  if (error) {
    ...
    db.query(
      'UPDATE roles_modules SET ? Where Role_Module_ID = ?',
...
router.delete('/delete/:id', (req, res) => {
  db.query(
    'DELETE from roles_modules Where Role_Module_ID = ?',
    [req.params.id],
    (err, result) => {
      if (err) {
        console.log(err);
